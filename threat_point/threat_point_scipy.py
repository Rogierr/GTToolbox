...
def maximin_p1_FD(x):
    ...
    left_result = np.multiply(res_p1[0], FD_left)
    right_result = np.multiply(res_p1[1], FD_right)
...
def maximin_p2_FD(x):
    ...
    FD_upper = 1 - 0.25*x[1]
    FD_bottom = 1 - 0.5*x[0] - 2/3*x[1]

    print(res_p2)

    upper_result = np.multiply(res_p2[0], FD_upper)
    bottom_result = np.multiply(res_p2[1], FD_bottom)
...
tp_p2_with_FD = minimize(payoff_p2_with_FD, x_p2, bounds=bnds, constraints=con, options={'disp': False})
